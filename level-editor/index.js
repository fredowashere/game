(function() {
    ...
    // ToDo: Should I clean this up once mapDataEditor gets destroyed?
    ...
    worldWrap.addEventListener("mousedown", mdevt => {
        ...
        if (mdevt.button === 0) {
            if (mdevt.target.classList.contains("tile")) {
                if (window.mapDataEditor.lmb === null) {
                    mdevt.target.removeAttribute("data-key");
                }
                else {
                    mdevt.target.setAttribute("data-key", window.mapDataEditor.lmb);
                }
            }
        }
        ...
        if (mdevt.button === 2) {
            if (mdevt.target.classList.contains("tile")) {
                if (window.mapDataEditor.rmb === null) {
                    mdevt.target.removeAttribute("data-key");
                }
                else {
                    mdevt.target.setAttribute("data-key", window.mapDataEditor.rmb);
                }
            }
        }
        ...
        // ToDo: Should this be cleaned by itself?
    ...
    window.mapDataEditor.load = mapData => {
